...
book_probe( tree_t * restrict ptree )
{
  ...
  if ( j != USHRT_MAX )
    {
      str_error = "normalization error (TenShougiModèòî’ä˚ïà.bin)";
      return -1;
    }
#endif
...
book_create( tree_t * restrict ptree )
{
  ...
  Out("\n  [TenShougiModèòî’ä˚ïà.csa]\n");
  
  iret = record_open( &record, "TenShougiModèòî’ä˚ïà.csa", mode_read, NULL, NULL );
  if ( iret < 0 ) { return iret; }
...
#if 1
  iret = record_open( &record, "TenShougiModèòî’âÒîä˚ïà.csa", mode_read, NULL, NULL );
  if ( iret < 0 ) { return iret; }
...
read_anti_book( tree_t * restrict ptree, record_t * pr )
{
  ...
  do {
    ...
    if ( result == -2 )
      {
	str_error = "no result in TenShougiModèòî’âÒîä˚ïà.csa";
	return -2;
      }
	       ...
	       int num_tmpfile )
{
  ...
  while ( istatus != record_eof ) {
    ...
    for ( ply = 0;; ply++ ) {
      ...
      if ( ! ply )
	{
	  black_bnz = strcmp( pr->str_name1, "TenShougiMod" ) ? 0 : 1;
	  white_bnz = strcmp( pr->str_name2, "TenShougiMod" ) ? 0 : 1;
	  if ( ! strcmp( pr->str_name1, "TenShougiMod" ) )
	    {
	      black_bnz   = 1;
	      nbnz_black += 1;
	    }
	  else { black_bnz = 0; }
	  if ( ! strcmp( pr->str_name2, "TenShougiMod" ) )
	    {
	      white_bnz   = 1;
	      nbnz_white += 1;
	    }
